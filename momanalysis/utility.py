...
class MonoLogger(logging.Logger):
    ...
    #    " ($BOLD%(filename)s$RESET:$RED%(lineno)d$RESET)",
    #    "  %(message)s",
    MONOFORMAT = formatter_message(FORMAT, True)
    def __init__(self, name):
        logging.Logger.__init__(self, name, logging.DEBUG)
...
if supports_color():
    #print("DEBUG: using color logger", flush=True)
    logger = ColoredLogger("momanalysis")
else:
    #print("DEBUG: using monochrome logger", flush=True)
    logger = MonoLogger("momanalysis")
    #logging.basicConfig()
    #logger = logging.getLogger("momanalysis")
